...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const pickerBranchId = computed(() => auth.isKasir ? auth.selectedBranch?.id : selectedBranch.value ? Number(selectedBranch.value) : void 0);
      ...
      if (unref(loading)) {
        ...
        if (unref(sales2).length === 0) {
          ...
          ssrRenderList(unref(sales2), (s) => {
            ...
            if (s.branch) {
              _push(`<span class="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded" data-v-bd2cb519>${ssrInterpolate(s.branch?.name)}</span>`);
            } else {
              _push(`<!---->`);
            }
            if (s.payment_type) {
            ...
            if (s.items?.length) {
              ...
              ssrRenderList(s.items, (item) => {
                _push(`<div class="flex items-center justify-between text-sm" data-v-bd2cb519><span class="text-gray-700 font-medium" data-v-bd2cb519>${ssrInterpolate(item.product?.name)}</span><span class="text-gray-400 text-xs" data-v-bd2cb519>x${ssrInterpolate(item.qty)} · ${ssrInterpolate(formatRupiah(item.subtotal))}</span></div>`);
              });
              _push(`<!--]--></div>`);
            } else {
              _push(`<!---->`);
...
//# sourceMappingURL=sales-CybYn-lG.js.map
